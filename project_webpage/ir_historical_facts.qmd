...
p <- retlong |> 
  ...
  labs(title="q-q plots of annual investment returns for individual assets",
       ...
       x = "theoretical quantiles for standard normal distribution",
...
The following plot shows a measure of mean reversion for S&P 500 total returns.

The measure is the coefficient on the AR(1) term from an ARIMA(1, 0, 0) model of total returns. When the AR(1) coefficient is negative, it indicates a mean-reverting process. This means that if a value is above the mean, the next value will tend to be closer to the mean, and vice versa. The magnitude of the coefficient indicates the speed of mean reversion: the larger the absolute value of the negative coefficient, the faster the process reverts to the mean. This coefficient is sometimes called *phi*.

In the plot below, the measure is calculated on a 30-year rolling basis: Each observation is the AR(1) coefficient estimated from an ARIMA(1, 0, 0) model of annual investment returns over the previous 30 years.


```{r}
#| label: plot-rolling-phi
#| eval: true
#| include: true

f2 <- rollify(ar1, window = 30, na_value=list(phi=NA_real_, pval=NA_real_), unlist = FALSE)
...
roll |> 
  ...
  ggtitle("Rolling 30-year measure of mean reversion in annual S&P 500 total returns. Negative values suggest mean reversion.",
          subtitle="Coefficients significantly different from zero at p < 0.05 (if any) are marked in red") +
  labs(x=NULL, y="phi",
       caption="Each observation is the AR(1) coefficient estimated from an ARIMA(1, 0, 0) model of annual investment returns over 30 years") +
...
### Efficient long-only portfolio q-q plot

```{r}

rts <- ts(returns |> dplyr::select(sp500))
...
xkurt <- e1071::kurtosis(pdata$porteff5050, type=1)

p <- pdata |> 
  ggplot(aes(sample=porteff5050)) +
  stat_qq_line() + 
  stat_qq(colour="blue", size=0.75) +
  geom_hline(yintercept = 0) +
  geom_hline(yintercept = mean(pdata$porteff5050), linetype="dashed") +
  geom_vline(xintercept = 0) +
  scale_x_continuous(name="theoretical quantiles for standard normal distribution",
                     breaks=seq(-4, 4, 0.5),
                     limits=c(-4, 4),
                     labels=label_number(accuracy=0.1)) +
  scale_y_continuous(breaks=c(seq(-2, 2, .05), mean(pdata$porteff5050)),
                     labels=label_percent(accuracy=0.1)) +
  ggtitle(label = "Efficient long-only portfolio with same mean as 50-50 stocks-bonds, 1950-22", 
          subtitle = paste0("Excess kurtosis = ", f_comma(xkurt, accuracy=.01))) +
