...
f <- function(seed){
  ...
  xkurt <- e1071::kurtosis(tvec)
  # tscaled <- scale_dist(tvec, 0, 1)
...
df |> 
  ...
  ggtitle(paste0("Distribution of excess kurtosis from 200 samples with degrees of freedom = ", f_comma(ir_df, accuracy=0.1)),
          paste0("Target value for excess kurtosis = ", f_comma(ir_xkurt, accuracy=0.01))) +
  theme_bw()
...
title1 <- paste0("Histogram of investment returns generated from a sample of ", f_comma(length(ir), accuracy=1), " observations")
title2 <- paste0("Drawn from a t distribution with mean= ",
