...
f <- function(data, vname, start, end){
  vec <- get_subset(vname, start, end, data)$lrc
  mod <- estimate_parameters(vec, model = "leverage", silent = TRUE)
  mod
}

f(stack, "us", "2010-01-31", "2020-05-02")

f2 <- function(dlist, vname, start, end){
  vec <- get_subset(vname, start, end, data)$lrc
  mod <- estimate_parameters(vec, model = "leverage", silent = TRUE)
  mod
}
...
res <- stack |> 
  # filter(vname=="us") |> 
  nest(.by=vname) |> 
  mutate(mod=purrr::map(data, \(x) f(x, start="2000-01-31", end="2020-05-02")))
...
dd <- res |> 
  # filter(vname=="intl") |> 
  mutate(z = map(mod, \(x) pluck(x, 1, "par.fixed"))) |> 
  unnest_wider(z)
  # mutate(summ = map(mod, pluck, 1) |> summary(report="transformed") |> list())
dd  
...
res |> 
  #filter(vname=="us")
  mutate(nobs=pluck(mod, 1, "nobs")),
         summ=summary(pluck(mod, 1), report = "transformed") |> list()) 
...
res <- stack |> 
  # filter(vname=="us") |> 
  nest(.by=vname) |> 
  summarise(mod=f(pick(date, lr), vname, start="2010-01-31", end="2020-05-02") |> list())
  
tmp <- res |> filter(vname=="us")
str(tmp$mod)
pluck(tmp$mod, 1, "nobs")
summary(tmp$mod)
summary(tmp$mod, )
tmp$mod
