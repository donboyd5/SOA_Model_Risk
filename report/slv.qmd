...
sims <- params2 |>
  ...
  mutate(pred=purrr::map(mod,
                         \(x) predict(x, steps=360, nsim = 10000, include_parameters = TRUE)),
...
sims |> 
  mutate(nrows=purrr::map(y, \(x) dim(x)[1]) |> unlist(),
         ncols=purrr::map(y, \(x) dim(x)[2]) |> unlist(),
         mean=purrr::map(y, \(x) mean(x) |> unlist())) |> 
  unnest(col=mean)
