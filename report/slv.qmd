...
weights2equity <- c(equity=.7, bonds=.3)
weights2bonds <- c(equity=.3, bonds=.7) 
...
aggrport2 <- combo2 |> 
  ...
  filter(date > min(date)) |> 
  mutate(equity_portret= equity * weights2equity["equity"] + bonds * weights2equity["bonds"],
         equity_portassets=exp(cumsum(equity_portret)),
         bonds_portret= equity * weights2bonds["equity"] + bonds * weights2bonds["bonds"],
...
port_monthly <- aggrport2 |> 
  ...
  mutate(vname="simport", 
         src="portfolio",
         lrequity=log(eqprice / lag(eqprice)),
         lrbonds=log(bondprice / lag(bondprice)))
