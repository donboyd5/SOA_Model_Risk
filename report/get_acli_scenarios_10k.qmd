...
(fnamesv <- setNames(
  fnames$fdesc,
  fnames$fname |> path_ext_remove() |> str_to_lower()
))
# fnamesv["aggr"]
...
scen_names <- tibble(scenario = runs) |>
  mutate(
    temp = str_replace(scenario, "_lbound", "__lbound"),
  ...
  mutate(
    parmdesc = case_when(
      ...
      parm ==
        "rho" ~ "Correlation between random shock to vol and random component of return (rho)",
  ...
  mutate(
    scendesc = case_when(
      bound == "lbound" ~ paste0(
        parmdesc,
        ": reduced to lower bound of 90% CI"
      ),
      bound == "ubound" ~ paste0(
        parmdesc,
        ": increased to upper bound of 90% CI"
      ),
...
saveRDS(scen_names, fs::path(GROOT, "data", "acli_scenario_names.rds"))

# scen_names <- readRDS(fs::path(GROOT, "data", "acli_scenario_names.rds"))
...
dirget <- function(dirname, fnames) {
  ...
  df2 <- df1 |>
    mutate(scenario = str_remove(dirname, "s_"), fname = path_file(fname)) |>
...
slong <- stack |>
  select(-fname) |>
  pivot_longer(cols = -c(scenario, fnbase, fdesc, sim), values_to = "asset") |>
  mutate(year = str_sub(name, 2, -1) |> as.integer() - 1) |>
  select(-name) |>
  arrange(scenario, fnbase, sim, year) |>
  mutate(
    logreturn = log(asset / lag(asset)), # log return, not arithmetic return
...
glimpse(slong)
saveRDS(slong, fs::path(GROOT, "data", "acli_scenarios.rds"))
