...
saveRDS(combo2, here::here("data", "aggr_assets_rawdaily.rds"))
...
combo2 <- readRDS(here::here("data", "aggr_assets_rawdaily.rds"))
...
#   arrange(date) |> 
#   mutate(across(c(equity, fixed), \(x) log(x / lag(x)))) |> # log returns
...
# lra  
# (wrong <- lra %*% wts)
# (port_aret <- (exp(lra) -1) %*% wts)
# (port_lret <- log(1 + port_aret))
# 
# lrport(lra, wts)
# 
# # what if lra is a tibble? 
# tlra <- as_tibble(lra, .name_repair = "universal")
# tlra <- as_tibble(lra, .name_repair = "minimal")
# tlra <- as_tibble(lra, .name_repair = make.names)
# as.matrix(tlra)
# 
# lrport(tlra, wts)
# lrport(tlra, wtsx)
...
aggrport2 <- combo2 |> 
  ...
  mutate(across(c(equity, fixed), \(x) log(x / lag(x)))) |> # log returns
  filter(date > min(date)) |> # drop first observation
  mutate(equity_portret = lrport(across(c(equity, fixed)), highequity_weights),
         equity_portassets=exp(cumsum(equity_portret)),
         fixed_portret = lrport(across(c(equity, fixed)), highfixed_weights),
         fixed_portassets=exp(cumsum(fixed_portret))) # note that this is end of period assets
