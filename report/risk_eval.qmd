...
riskmeasures1 <- result |> 
  ...
  # filter(scenario %in% c("baseline", "phi_m.05")) |> 
  unnest(col=penmod) |> 
  summarise(mean=first(mean_logreturn),
            sd=first(sd_logreturn),
...
riskmeasures <- riskmeasures1 |> 
  pivot_longer(-c(scenario, porttype, version, mean, sd),
               names_to = "measure") |> 
...
norms <- riskmeasures |> 
  filter(scenario=="baseline") |> 
  select(scenario, porttype, measure, rnormal)  |> 
  mutate(type="normal") |> 
  pivot_wider(names_from = measure, values_from = rnormal)

mods <- riskmeasures |> 
  filter(scenario=="baseline") |> 
  select(scenario, porttype, measure, model)  |> 
  mutate(type="model") |> 
  pivot_wider(names_from = measure, values_from = model)

tots <- bind_rows(norms, mods) |> 
  arrange(desc(type), porttype)

tots |> 
  select(porttype, type, vloss_a95t10, verc_a95t10, probfr_fr70t10) |> 
  arrange(porttype, desc(type)) |> 
  gt() |> 
  tab_header(
  ...
  fmt_number(columns = c(vloss_a95t10, verc_a95t10, probfr_fr70t10),
             decimals = 3)
