...
portfolios <- wtdslong |> 
  # mutate(return_share=logreturn * weight) |> 
  mutate(areturn=exp(logreturn) - 1, # arithmetic return for the asset class
         areturn_share=areturn * weight) |> 
  summarise(areturn=sum(areturn_share), .by=c(scenario, sim, year, porttype)) |> # arithmetic returns for the portfolios
  mutate(logreturn=log(1 + areturn),  # log returns for the portfolios
