...
slv_sims <- readRDS(fs::path(GROOT, "data", "slvsims.rds"))
slv_sims
glimpse(slv_sims)
...
df <- readRDS(fs::path(GROOT, "data", "acli_portfolios.rds"))
...
mean_annual_logreturn_highequity <- .0615 # same as highequity acli portfolio updated for 70/30 portfolio, from 0.625 at ~72/28
mean_annual_logreturn_highfixed <- 0.0418 # same as highfixed acli portfolio updated for 30/70 portfolio, from 0.0499 at ~ 46/54

c(
  mean_annual_logreturn_highequity,
  acli_means$lr[acli_means$porttype == "highequity"]
) |>
  round(4)
c(
  mean_annual_logreturn_highfixed,
  acli_means$lr[acli_means$porttype == "highfixed"]
) |>
  round(4)
...
slv2 <- slv_sims |>
  ...
  ) |> # add target mean annual log return to innovations
  # base stats on the targeted return, not the raw innovations
  select(-lrmat) |>
  mutate(
    nsims = map(model, \(x) nrow(x)) |> pluck(1),
  ...
  mutate(
    ...
    rnormal = get_normal(mean_model, sd_model, nsims, nyears, seed = 1234) |>
      list()
  ) |>
  pivot_longer(
    c(model, constant, rnormal),
    names_to = "modtype",
    values_to = "lrmat"
  ) |> # lrmat for log returns matrix
  group_by(scenario, porttype, modtype) |>
  mutate(mean = mean(unlist(lrmat)), sd = sd(unlist(lrmat))) |>
  select(
    scenario,
    porttype,
    modtype,
...
saveRDS(slv2, fs::path(GROOT, "data", "slv_and_comparison_portfolios.rds"))
...
acli <- readRDS(fs::path(GROOT, "data", "acli_portfolios.rds"))
glimpse(acli)
count(acli, scenario)
count(acli, porttype)
...
saveRDS(port2, fs::path(GROOT, "data", "acli_and_comparison_portfolios.rds"))
...
slv_scen <- readRDS(fs::path(
  GROOT,
  "data",
  "slv_and_comparison_portfolios.rds"
))
acli_scen <- readRDS(fs::path(
  GROOT,
  "data",
  "acli_and_comparison_portfolios.rds"
))

all_scen <- bind_rows(
  slv_scen |> mutate(framework = "slv"),
...
saveRDS(
  all_scen,
  fs::path(GROOT, "data", "allscen_and_comparison_portfolios.rds")
)

all_scen <- readRDS(fs::path(
  GROOT,
  "data",
