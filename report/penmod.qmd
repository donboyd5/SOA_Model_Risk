...
make_irmat(test$sim, test$year, test$wtdreturn)

test <- acliport |>
  filter(scenario=="baseline", porttype=="highequity", sim %in% 1:10, year %in% 0:5)


prep <- acliport |>
  filter(year != 0) |> 
  summarise(irmat=make_irmat(sim, year, wtdreturn), .by=c(scenario, porttype))
...
result <- prep  |> 
  ...
  mutate(penmod = 
           purrr::map(seq_along(irmat),
                      ~penmod2(
                        ir = irmat[[.x]])))
result

saveRDS(result, here::here("data", "acli_penmod.rds"))
...
apm <- readRDS(here::here("data", "acli_penmod.rds"))
