...
mod_labs <- c("Constant", "Random\nnormal", "Gaussian SLV", "Skewed SLV", "Leverage SLV", "Complex ESG")
...
base_flipped |> 
  ...
  cols_label(typef = html("Constant,<br>Random normal, or<br>modeled returns"),
...
base_flipped |> 
  ...
  scale_y_continuous(name="% Change",
                     breaks = seq(-1, 1, .1), labels = label_percent(accuracy=1)) +
  labs(x=NULL) +
  theme_bw() +
...
base_flipped |> 
  ...
  labs(x=NULL) +
  theme_bw() +
  ggtitle("Contribution % change year 10 vs. year 1, 5th percentile",
          subtitle = "Alternative models, mean log return ~6.25%") +
  facet_wrap(~porttype, ncol = 1, scales = "fixed")
