...
mod_labs <- c("Deterministic", "Random\nnormal", "Gaussian SV", "Skewed SV", "Leverage SV", "Detailed ESG")
...
gtsave(tab, "risk_measures_highequity_table.png", path = here::here("report", "results"))
...
gtsave(tab, "risk_measures_highfixed_table.png", path = here::here("report", "results"))
...
tab <- parm_flipped |> 
  ...
  cols_hide(c(type)) |> 
  tab_header(
    title = html("Selected Risk Measures for Alternative Parameter Assumptions for the Detailed ESG, High Equity Portfolio"),
    # subtitle = html("Normal returns calibrated to have same mean and SD as Gaussian SV model")
  ) |>
  cols_label(rn="Row number",
             typef = html("Deterministic,<br>Random normal, or<br>modeled returns"),
...
vheight <- 744 # 744
zoom <- 1
gtsave(tab, "parm_risk_measures_highequity_table.png", path = here::here("report", "results"),
       vwidth = vwidth,
       vheight = vheight,
       zoom = zoom)
...
tab <- parm_flipped |> 
  ...
  cols_hide(c(type)) |> 
  tab_header(
    title = html("Selected Risk Measures for Alternative Parameter Assumptions for the Detailed ESG, High Fixed-Income Portfolio"),
    # subtitle = html("Normal returns calibrated to have same mean and SD as Gaussian SV model")
  ) |>
  cols_label(rn="Row number",
             typef = html("Deterministic,<br>Random normal, or<br>modeled returns"),
...
p <- range_data |>
  ...
  ggtitle("VaR of Asset Loss Under Alternative ESG Model Parameter Assumptions",
...
p <- range_data |>
  ...
  theme_bw() +
  facet_wrap(~ porttype, scales = "fixed",  ncol = 1) +
  ggtitle("VaR of Employer Contribution Increase Under Alternative ESG Model Parameter Assumptions",
          subtitle = "Based on upper-bound and lower-bound values of important model parameters")
p
