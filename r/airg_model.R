...
a <- proc.time()
arnorm_cor <- intgen_rancorr(
  nyears = runparams$nyears,
  nsims = runparams$nsims,
  runparams,
  seed = 1234
)
b <- proc.time()
...
icurve <- runparams$init_curve |>
  dplyr::select(-c(date, curvenum)) |>
  as.numeric()
runparams$init_curve_NS <- interpolateNS(r1 = genrate1, r20 = genrate2)
